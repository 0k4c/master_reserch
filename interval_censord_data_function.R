#generate_interval_censored_data_2arm.R の修正版
...
generate_interval_censored_data_2arm <- function(n=100, K=5, p_dropout="Medium", p_exact=0, d=1){
  ...
  # エラーチェック
  if (!is.element(n, c(100,200,400))) 
    stop("only (100,200,400) is permitted for n")
...
# 右点代入法（Right-point Assignment）
rightpoint_assignment <- function(matrix){
  # 結果を格納するデータフレームを初期化
  result <- array(numeric(nrow(matrix)*2), dim=c(nrow(matrix),2))
  
  # 区間打ち切りデータを右端点に代入（delta=1: イベント発生）
